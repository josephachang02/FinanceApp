...
//select all inputs
const allPersonalInput = document.querySelectorAll('.personalInput');
...
form.addEventListener('submit', function(event) {
    ...
    let allPersonalTotal = total; //document.getElementById('NecessaryTotal').value;
        window.location.href = `PersonalSpendings.html?subtotal=${allPersonalTotal}`;
    });

const incomeDisplay = document.getElementById('incomeDisplay');

// Display the income in the <p> element
incomeDisplay.textContent = `Calculated Income: $${income}, Calculated Expenses: $${expenses}`;